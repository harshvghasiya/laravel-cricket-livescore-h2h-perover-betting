                    ...
                    <div class="ps-3">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0 p-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">{{trans('message.index_city_breadcrum')}}</li>
                            </ol>
                        </nav>
                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <select name="country_id" class="form-control select2 country">
                                            <option value="">{{trans('message.select_country_label')}}</option>
                                            @foreach(\App\Models\Country::getCountryDropDown() as $key=>$value)
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row m-4">
                            <div class="col-md-6">
                              <div class="btn-group">
                                <a href="{{route('admin.city.create')}}" id="sample_editable_1_new" class="btn btn-primary">
                                {{trans('message.add_new')}} 
                                </a>
                              </div>
                              
                                <div class="btn-group d-none">
                                  <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.city.delete_all')}}')">{{trans('message.delete_all_label')}}
                                  </a>
                                </div>
                            
                              <div class="btn-group d-none">
                                <a class="btn btn-warning" style="color: white;" onclick="statusAll('table_form','{{route('admin.city.status_all')}}')">{{trans('message.status_all_label')}}
                                </a>
                              </div>
                            </div>
                        </div>
...
@section('javascript')
  ...
  function RESET_FORM(){
    ...
    $("select[name=country_id]").trigger('change').val(''); 
    $("select[name=state_id]").trigger('change').val(''); 
        oTable.draw();
        return false;
  }
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.city.single_status_change')}}",
