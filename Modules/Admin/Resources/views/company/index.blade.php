                    ...
                    <div class="ps-3">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0 p-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">{{trans('message.index_company_breadcrum')}}</li>
                            </ol>
                        </nav>
                    </div>
                        ...
                        <div class="card-body">
                            <form method="POST" id="search-form" class="form-inline" role="form">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <select name="company_category" class="form-control select2">
                                            <option value="">{{trans('message.select_company_category_label')}}</option>
                                            @foreach(\App\Models\CompanyCategory::CompanyCategoryDropDown() as $key=>$value)
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row m-4">
                            <div class="col-md-6">
                              <div class="btn-group">
                                <a href="{{route('admin.company.create')}}" id="sample_editable_1_new" class="btn btn-primary">
                                {{trans('message.add_new')}} 
                                </a>
                              </div>
                              
                                <div class="btn-group d-none">
                                  <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.company.delete_all')}}')">{{trans('message.delete_all_label')}}
                                  </a>
                                </div>
                            
                              <div class="btn-group d-none">
                                <a class="btn btn-warning" style="color: white;" onclick="statusAll('table_form','{{route('admin.company.status_all')}}')">{{trans('message.status_all_label')}}
                                </a>
                              </div>
                            </div>
                        </div>
                            ...
                            <table id="users-table" class="table table-striped table-bordered" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{trans('message.company_category')}}</th>
...
@section('javascript')
  ...
  var oTable = $('#users-table').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('admin.company.any_data') !!}',
            data: function (d) {
                d.name = $('input[name=name]').val();
                d.status = $('select[name=status]').val();
                d.company_category = $('select[name=company_category]').val();
              }
          },
      columns: [
  ...
  function RESET_FORM(){
    ...
    $("select[name=company_category]").trigger('change').val(""); 
        oTable.draw();
        return false;
  }
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.company.single_status_change')}}",
