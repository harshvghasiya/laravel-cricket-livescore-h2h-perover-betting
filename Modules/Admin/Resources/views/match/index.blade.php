                    ...
                    <div class="ps-3">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0 p-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">{{trans('message.index_match_breadcrum')}}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row m-4">
                            <div class="col-md-6">
                             {{--  <div class="btn-group ">
                                <a href="{{route('admin.match.create')}}" id="sample_editable_1_new" class="btn btn-primary">
                                {{trans('message.add_new')}} 
                                </a>
                              </div> --}}
                              
                                <div class="btn-group d-none">
                                  <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.match.delete_all')}}')">{{trans('message.delete_all_label')}}
                                  </a>
                                </div>
                            
                              <div class="btn-group d-none">
                                <a class="btn btn-warning" style="color: white;" onclick="statusAll('table_form','{{route('admin.match.status_all')}}')">{{trans('message.status_all_label')}}
                                </a>
                              </div>
                            </div>
                        </div>
                        <div class="table-responsive">
                            ...
                            <table id="users-table" class="table table-striped table-bordered" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>{{trans('message.eid')}}</th>
                                        <th>{{trans('message.team_1_odd')}}</th>
                                        <th>{{trans('message.team_2_odd')}}</th>
                                        <th>{{trans('message.draw')}}</th>
                                        <th>{{trans('message.team_name1')}}</th>
                                        <th>{{trans('message.team_name2')}}</th>
...
@section('javascript')
  ...
  var oTable = $('#users-table').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('admin.match.any_data') !!}',
            data: function (d) {
      ...
      columns: [
          { data: 'eid',orderable: false,searchable: false},
          { data: 'team_1_odd'},
          { data: 'team_2_odd'},
          { data: 'draw'},
          { data: 'team_name1'},
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.match.single_status_change')}}",
