                    ...
                    <div class="ps-3">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0 p-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">{{trans('message.index_support_breadcrum')}}</li>
                            </ol>
                        </nav>
                    </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row m-4">
                            <div class="col-md-6">
                              <div class="btn-group">
                                <a href="{{route('admin.support.create')}}" id="sample_editable_1_new" class="btn btn-primary">
                                {{trans('message.add_new')}} 
                                </a>
                              </div>
                              
                                <div class="btn-group d-none">
                                  <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.support.delete_all')}}')">{{trans('message.delete_all_label')}}
                                  </a>
                                </div>
                            
                              <div class="btn-group d-none">
                                <a class="btn btn-warning" style="color: white;" onclick="statusAll('table_form','{{route('admin.support.status_all')}}')">{{trans('message.status_all_label')}}
                                </a>
                              </div>
                            </div>
                        </div>
...
@section('javascript')
  ...
  var oTable = $('#users-table').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('admin.support.any_data') !!}',
            data: function (d) {
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.support.single_status_change')}}",
