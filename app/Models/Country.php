...
class Country extends Model
{
    ...
    */
    ...
    { 
        if ($id ==null) {   
           $res=new self;
           $msg=trans('message.country_added_successfully');
           $description=trans('message.new_country_added');


        }else{
            $id=Crypt::decrypt($id);
            $res=self::find($id);
            $msg=trans('message.country_updated_successfully');
        ...
        $res->save();
        if ($id ==null) {
          PanelActivtyStore($res->id,$res->name,$description,COUNTRY_ROUTE_NAME());
        }
    ...
    */
    public function getListData($request)
    {
        ...
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                  ...
                  $string ='<a title="'.trans('message.edit_country').'" href="'.route('admin.country.edit',Crypt::encrypt($data->id)).'" class="btn btn-xs btn-primary"><i class="fadeIn animated bx bx-message-square-edit"></i></a>';
    ...
    public function deleteAll($r)
    {
     ...
     $msg="";
      foreach ($input['checkbox'] as $key => $c) {
           ...
           $checkModule = \App\Models\State::where('country_id',Crypt::decrypt($c))->get();
          ...
          if (!$checkModule->isEmpty()) {
            ...
            $obj->delete();
            $msg .= trans('message.country_delete_message_label');
            $status = 1;
    ...
    public function statusAll($r){
      ...
      return response()->json(['success' => 1, 'msg' => trans('message.country_delete_message_label')]);
    } 
